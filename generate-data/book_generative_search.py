...
db_name = "Book"
client = weaviate.connect_to_custom(http_host="localhost", http_port=8080, grpc_port=50052, http_secure=False, grpc_host="localhost", grpc_secure=False, additional_config=AdditionalConfig(
    timeout=Timeout(init=2, query=200, insert=120)  # Values in seconds
))

print(client.is_connected())

book_collection = client.collections.get(name=db_name)
...
response = book_collection.generate.near_text(
    query=user_input,
    limit=2,
    single_prompt="Explain why this book might be interesting to read. The book's title is {title} and is in the genre: {categories}."
